	...
	<div class="d-flex justify-content-between p-3" style="height: 15%">
		...
		<c:if test="${!userDisabled}">
			...
			<c:if test="${sameWriter == true || isAdmin}">
				<button class="btn rounded-pill"
					onClick="location.href='/writers/update/${writer.id}'"
					style="color: white;height: fit-content;background-color: ${isAdmin?'#8a4380':'#af3248'}">Actualizar
					perfil</button>
				<button class="btn rounded-pill"
					onClick="location.href='/writers/delete/${writer.id}'"
		...
		<c:if test="${userDisabled}">
			...
			<c:if test="${isAdmin}">
				<button class="btn rounded-pill"
					...
					onclick="location.href='/writers/activate/${writer.id}'">Activar
					escritor</button>
			</c:if>
		</c:if>
	</div>
			...
			<div class="d-flex justify-content-between w-100"
				...
				<c:choose>
					<c:when test="${sameWriter == true && !isAdmin}">
						<button class="btn rounded-pill"
							onClick="location.href='/stories/create'"
							style="color: white; background-color: #af3248">Crear
				...
				<div style="width: 50%; border-color: #af3248; border-style: solid; border-width: 0 2px 0 0;">
					<div style="margin: 1% 0">
						<div class="d-flex flex-wrap ">
							<h5 class="p-2 rounded-pill" style="background-color: #3e3e3e">Nombre
								de usuario</h5>
						</div>
				...
				<div style="padding: 2% 0; width: 30%; margin-left: 3%">
					...
					<c:forEach items="${stories}" var="story">
						<div class="d-flex align-items-center justify-content-evenly"
							...
							onClick="location.href='/stories/show/${story.id}'">
